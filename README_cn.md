...
- `callback.onDetected(isProtected)`：检测到加固状态时触发（`true` 表示存在加固/加壳保护，`false` 表示未加固）
- `callback.onUnpacked()`：存在加固/加壳保护时，后续检测到原始应用已加载/解包完成时触发
...
- **不做壳类型识别**：只做“是否存在加固/加壳保护”与“何时解包完成”的通用判断。
